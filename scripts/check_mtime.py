...
def test_mtime():
    ...
    if modal.is_local():
        ...
        with myvol.batch_upload(force=True) as batch:
            batch.put_file(BytesIO("file text written with modal batch.put_file()".encode()), new_file_path)
        ...
        with myvol.batch_upload(force=True) as batch:
            batch.put_file(BytesIO("file text written with modal batch.put_file()".encode()), new_file_path2)
