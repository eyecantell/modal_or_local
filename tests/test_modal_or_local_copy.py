...
def test_copy_dir_from_volume_to_local():
    ...
    print("\n\nRunning test_copy_dir_from_volume_to_local", "locally" if modal.is_local() else "remotely")

    if not modal.is_local():
        raise RuntimeError("Cannot run test_copy_dir_from_volume_to_local remotely since /tmp is not mounted remotely")
    ...
    for prefix in ["a", "b", "c"]:
        test_json_data = json.loads('{"' + prefix + '": "' + prefix + '_val"}') # e,g. {"a":"a_val"}
        test_file_full_path_volume_one = os.path.join(temp_dir_volume_one, prefix + ".json")
        mocal_for_volume_one.write_json_file(test_file_full_path_volume_one, test_json_data) 
        assert mocal_for_volume_one.file_or_dir_exists(test_file_full_path_volume_one), f"Could not find file created on volume one {test_file_full_path_volume_one=}"
        expected_files_relative_path.append(os.path.basename(test_file_full_path_volume_one))

    # Create aa.json, bb.json, cc.json in /test_mnt_dir_one/test_copy_file_from_volume_to_local_dir/subdir
    for prefix in ["aa", "bb", "cc"]:
        test_json_data = json.loads('{"' + prefix + '": "' + prefix + '_val"}') # e,g. {"aa":"aa_val"}
        test_file_full_path_volume_one = os.path.join(temp_dir_volume_one, "subdir", prefix + ".json")
        mocal_for_volume_one.write_json_file(test_file_full_path_volume_one, test_json_data) 
        assert mocal_for_volume_one.file_or_dir_exists(test_file_full_path_volume_one), f"Count not find file created on local {test_file_full_path_volume_one=}"
        expected_files_relative_path.append(os.path.join("subdir", os.path.basename(test_file_full_path_volume_one)))

    # Add /test_mnt_dir_one/test_copy_file_from_volume_to_local_dir/empty_subdir as an empty dir
    test_empty_dir_full_path_volume_one = os.path.join(temp_dir_volume_one, "empty_subdir")
    ...
    copy_dir(source_mocal=mocal_for_volume_one, source_dir_full_path=temp_dir_volume_one, \
              destination_mocal=mocal_for_local, destination_full_path=temp_dir_local)
    
    # Make sure the expected files all exist
    for file_relative_path in expected_files_relative_path:
        assert mocal_for_local.file_or_dir_exists(os.path.join(temp_dir_local, temp_dir_name, file_relative_path))
    ...
    assert not mocal_for_volume_one.file_or_dir_exists(temp_dir_volume_one)
    assert not mocal_for_local.file_or_dir_exists(temp_dir_local)
    print("Running test_copy_dir_from_volume_to_local", "locally" if modal.is_local() else "remotely", "finished")

#
#  Copy tests. Test modal_or_local_copy.copy()
#

@app.function(image=image, volumes={mocal_for_volume_one.volume_mount_dir: mocal_for_volume_one.volume, mocal_for_volume_two.volume_mount_dir: mocal_for_volume_two.volume}) 
def test_copy():
    ...
    # Copy the file from volume one to volume two, naming the destination file explicitly
    destination_file_full_path = os.path.join(temp_dir_volume_two, "named_dest_test.json")
    copy(source_mocal=mocal_for_volume_one, source_path=test_file_full_path_volume1, \
              destination_mocal=mocal_for_volume_two, destination_path=destination_file_full_path)
    ...
    for prefix in ["a", "b", "c"]:
        test_json_data = json.loads('{"' + prefix + '": "' + prefix + '_val"}') # e,g. {"a":"a_val"}
        test_file_full_path_volume_two = os.path.join(dir_to_copy_full_path_volume_two, prefix + ".json")
        mocal_for_volume_two.write_json_file(test_file_full_path_volume_two, test_json_data) 
        assert mocal_for_volume_two.file_or_dir_exists(test_file_full_path_volume_two), f"Could not find file created on volume two {test_file_full_path_volume_two=}"
        expected_files_relative_path.append(test_file_full_path_volume_two.replace(temp_dir_volume_two + '/',"",1))

    copy(source_mocal=mocal_for_volume_two, source_path=dir_to_copy_full_path_volume_two, \
              destination_mocal=mocal_for_volume_one, destination_path=temp_dir_volume_one)
    
    # Make sure the expected files all exist
    for file_relative_path in expected_files_relative_path:
        test_file_full_path_volume_one = os.path.join(temp_dir_volume_one, file_relative_path)
        print(f"{test_file_full_path_volume_one=}")
        assert mocal_for_volume_one.file_or_dir_exists(test_file_full_path_volume_one), f"Did not find file on volume one: {test_file_full_path_volume_one=}"
    
    print("Running test_copy", "locally" if modal.is_local() else "remotely", "finished")
